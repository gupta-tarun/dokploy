...
detect_version() {
    ...
    local repo="${DOKPLOY_REPO:-gupta-tarun/dokploy}"
    
    # If no version specified, get latest stable version from GitHub releases
    if [ -z "$version" ]; then
        echo "Detecting latest stable version from GitHub repository: ${repo}..." >&2
...
install_dokploy() {
    ...
    IMAGE_NAME="${DOKPLOY_IMAGE:-guptatarun/dokploy}"
    DOCKER_IMAGE="${IMAGE_NAME}:${VERSION_TAG}"
    
    echo "Installing Dokploy version: ${VERSION_TAG} from image: ${IMAGE_NAME}"
    if [ "$(id -u)" != "0" ]; then
